    ...
    if (!isset($_POST['csrf_token']) || !verifyCSRFToken($_POST['csrf_token'])) {
        ...
        if ($accion === 'crear') {
            ...
            if (empty($nombre)) {
                ...
                if ($stmt->get_result()->num_rows > 0) {
                    ...
                    $stmt = $conn->prepare("INSERT INTO proveedores (nombre, contacto, telefono, notas, estado) VALUES (?, ?, ?, ?, 'activo')");
                    $stmt->bind_param("ssss", $nombre, $contacto, $telefono, $notas);
                    if ($stmt->execute()) {
                        $mensaje = 'Proveedor creado correctamente';
                        logActivity('proveedor_creado', 'Proveedor: ' . $nombre);
                    } else {
        ...
        if ($accion === 'editar') {
            ...
            if (empty($nombre) || $id <= 0) {
                ...
                $stmt = $conn->prepare("SELECT id FROM proveedores WHERE nombre = ? AND id != ?");
                $stmt->bind_param("si", $nombre, $id);
                $stmt->execute();
                if ($stmt->get_result()->num_rows > 0) {
                    $error = 'Ya existe otro proveedor con ese nombre';
                } else {
                    $stmt = $conn->prepare("UPDATE proveedores SET nombre = ?, contacto = ?, telefono = ?, notas = ? WHERE id = ?");
                    $stmt->bind_param("ssssi", $nombre, $contacto, $telefono, $notas, $id);
                    if ($stmt->execute()) {
                        $mensaje = 'Proveedor actualizado correctamente';
                        logActivity('proveedor_editado', 'Proveedor ID: ' . $id);
                    } else {
        ...
        if ($accion === 'cambiar_estado') {
            ...
            if ($stmt->execute()) {
                ...
                logActivity('proveedor_estado', 'Proveedor ID: ' . $id . ' -> ' . $estado);
            } else {
...
// Obtener proveedores con conteo de cuentas
$proveedores = $conn->query("
    SELECT p.*, COUNT(c.id) as total_cuentas 
    FROM proveedores p 
    LEFT JOIN cuentas c ON c.proveedor_id = p.id 
    GROUP BY p.id 
...
<div class="min-h-screen flex bg-light-bg dark:bg-dark-bg">
    ...
    <aside id="sidebar" class="fixed inset-y-0 left-0 z-50 w-64 bg-light-card dark:bg-dark-card border-r border-light-border dark:border-dark-border transform -translate-x-full lg:translate-x-0 transition-transform duration-300">
        ...
        <nav class="p-4 space-y-1 overflow-y-auto" style="max-height: calc(100vh - 200px);">
            ...
            <a href="proveedores.php" class="flex items-center gap-3 px-4 py-3 rounded-lg bg-primary-50 dark:bg-primary-900/20 text-primary-600 dark:text-primary-400 font-medium">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="flex-1 lg:ml-64">
        ...
        <header class="h-16 bg-light-card dark:bg-dark-card border-b border-light-border dark:border-dark-border flex items-center justify-between px-4 lg:px-6">
            ...
            <div class="flex items-center gap-4">
                ...
                <h1 class="text-lg font-semibold text-slate-800 dark:text-white">Proveedores</h1>
            </div>
        ...
        <main class="p-4 lg:p-6">
            ...
            <div class="bg-light-card dark:bg-dark-card rounded-xl border border-light-border dark:border-dark-border overflow-hidden">
                ...
                <div class="p-4 border-b border-light-border dark:border-dark-border">
                    <h2 class="font-semibold text-slate-800 dark:text-white">Lista de Proveedores</h2>
                    <p class="text-sm text-slate-500 dark:text-slate-400">Total: <?php echo count($proveedores); ?> proveedores</p>
                </div>
                    ...
                    <table class="w-full">
                        <thead class="bg-slate-50 dark:bg-slate-800/50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">Contacto</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">Telefono</th>
                                <th class="px-4 py-3 text-center text-xs font-medium text-slate-500 dark:text-slate-400 uppercase">Cuentas</th>
                        ...
                        <tbody class="divide-y divide-light-border dark:divide-dark-border">
                            ...
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                                <td class="px-4 py-3">
                                    <span class="font-medium text-slate-800 dark:text-white"><?php echo e($proveedor['nombre']); ?></span>
                                </td>
                                <td class="px-4 py-3 text-sm text-slate-600 dark:text-slate-300">
                                    <?php echo e($proveedor['contacto'] ?: '-'); ?>
                                </td>
                                <td class="px-4 py-3 text-sm text-slate-600 dark:text-slate-300">
                                    <?php if ($proveedor['telefono']): ?>
                                    <a href="https://wa.me/<?php echo preg_replace('/[^0-9]/', '', $proveedor['telefono']); ?>" target="_blank" class="flex items-center gap-1 text-green-600 dark:text-green-400 hover:underline">
                                ...
                                <td class="px-4 py-3 text-center">
                                    <span class="inline-flex px-2 py-1 text-xs font-medium rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400">
                                        <?php echo $proveedor['total_cuentas']; ?>
                                    </span>
                                </td>
                                <td class="px-4 py-3 text-center">
                                    <span class="inline-flex px-2 py-1 text-xs font-medium rounded-full <?php echo $proveedor['estado'] === 'activo' ? 'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400' : 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400'; ?>">
                                        <?php echo ucfirst($proveedor['estado']); ?>
                                    </span>
                                </td>
                                <td class="px-4 py-3">
                                    <div class="flex items-center justify-center gap-2">
                                        <button onclick="openModal('editar', <?php echo htmlspecialchars(json_encode($proveedor)); ?>)" class="p-2 text-slate-500 hover:text-primary-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition-colors" title="Editar">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="md:hidden divide-y divide-light-border dark:divide-dark-border">
                    ...
                    <div class="p-4">
                        <div class="flex items-start justify-between mb-2">
                            <div>
                                <h3 class="font-medium text-slate-800 dark:text-white"><?php echo e($proveedor['nombre']); ?></h3>
                                <p class="text-sm text-slate-500 dark:text-slate-400"><?php echo e($proveedor['contacto'] ?: 'Sin contacto'); ?></p>
                            </div>
                            <span class="inline-flex px-2 py-1 text-xs font-medium rounded-full <?php echo $proveedor['estado'] === 'activo' ? 'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400' : 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400'; ?>">
                                <?php echo ucfirst($proveedor['estado']); ?>
                            </span>
                        ...
                        <div class="flex items-center gap-4 text-sm text-slate-500 dark:text-slate-400 mb-3">
                            <?php if ($proveedor['telefono']): ?>
                            <a href="https://wa.me/<?php echo preg_replace('/[^0-9]/', '', $proveedor['telefono']); ?>" target="_blank" class="flex items-center gap-1 text-green-600 dark:text-green-400">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                                </svg>
                                <?php echo e($proveedor['telefono']); ?>
                            ...
                            <?php endif; ?>
                            <span class="text-blue-600 dark:text-blue-400"><?php echo $proveedor['total_cuentas']; ?> cuentas</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <button onclick="openModal('editar', <?php echo htmlspecialchars(json_encode($proveedor)); ?>)" class="flex-1 px-3 py-2 text-sm text-primary-600 dark:text-primary-400 bg-primary-50 dark:bg-primary-900/20 rounded-lg hover:bg-primary-100 dark:hover:bg-primary-900/30 transition-colors">
                                Editar
                            </button>
                            <form method="POST" class="flex-1" onsubmit="return confirm('¿Cambiar estado?')">
                                ...
                                <button type="submit" class="w-full px-3 py-2 text-sm text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 rounded-lg hover:bg-amber-100 dark:hover:bg-amber-900/30 transition-colors">
                                    <?php echo $proveedor['estado'] === 'activo' ? 'Desactivar' : 'Activar'; ?>
                                </button>
                            </form>
                        </div>
                    </div>
...
<div id="modal" class="fixed inset-0 z-50 hidden">
    ...
    <div class="absolute inset-4 sm:inset-auto sm:top-1/2 sm:left-1/2 sm:-translate-x-1/2 sm:-translate-y-1/2 sm:w-full sm:max-w-md bg-light-card dark:bg-dark-card rounded-xl shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <div class="flex items-center justify-between p-4 border-b border-light-border dark:border-dark-border">
            <h3 id="modalTitle" class="font-semibold text-slate-800 dark:text-white">Nuevo Proveedor</h3>
        ...
        <form method="POST" class="p-4">
            ...
            <div class="space-y-4">
                ...
                <div>
                    <label for="contacto" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
                        Contacto
                    </label>
